...
def _step_code_hash(*filenames: str) -> str:
    ...
    When the source code for a specific step changes, its hash changes,
    invalidating only that step's cache. Other steps remain cached.
    """
    h = hashlib.md5()
...
class UnifiedPipelineController:
    ...
    def create_pipeline(self) -> Any:
        ...
        # Install custom hash callback for per-step cache invalidation
        _install_cache_hash_callback()
    ...
    def _add_single_dataset_steps(self):
        ...
        p.add_step(
            ...
            parameter_override={
                ...
                "_meta/force_cache_upload": str(not cfg.execution.cache_data_steps).lower(),
            },
