...
3. ✅ **RAN 2026-06-08 — subnuclear structure is a modest, *class-dependent* contributor.** A LightGBM floor on per-nucleus summary stats reaches **0.497** macro-F1; +whole-patch context **0.563** — within **0.056** of the CNN's 0.619, so the deep net's edge is mostly *context*, not fine chromatin (top tree features are nucleus geometry + context intensity; no chromatin-texture feature dominates). IG attribution concentration is ~1.0 overall but splits by class: **Immune 1.49 (subnuclear-driven)**, Stromal 1.17, Epithelial 0.86 & Endothelial 0.77 (context-driven). **Decision: skip the deferred phase-A retrain/masking matrix** — subnuclear signal isn't dominant enough to justify the GPU. *Thesis-relevant:* immune — the thesis's target compartment — is the one class where DAPI subnuclear structure carries the most signal, so the mechanistic premise lands in the best-case compartment. (code in `main`, ex-`feat/subnuclear-triangulation`; outputs `pipeline_output/subnuclear_2026_06/`)
...
3. ✅ **DONE 2026-06-08 — subnuclear-triangulation ran.** Subnuclear structure is modest + class-dependent; immune is subnuclear-driven (IG concentration 1.49) while epi/endo are context-driven; floor nuc+ctx 0.563 vs CNN 0.619. Phase-A retrain matrix skipped. See §C.3.
