...
**Status of this review's own fixes:** the P0/P1 items marked `FIXED` below were corrected in this session with tests; the rest are specced in §5.
...
| B5 | P1 | `scripts/breast_pooled_train.py:184-185` | Incomplete determinism: no cuDNN-deterministic, no DataLoader `generator`/`worker_init_fn`, Python `random` unseeded. Same seed ≠ same result → a 2% delta is inside noise. | Full `set_seed()` + deterministic DataLoader; then ≥3 seeds on the winner. | PLANNED (§5 P1) |
...
**Phase 0 — landed this session (P0/P1 surgical fixes, tested):** B1, B3, B4. See git log on `feat/nucleus-qc-scorer`.

**Phase 1 — experiment correctness (do before any A/B/C run):**
- B2 + B7: persist `cell_id` registry in the production builder; cast-guard the centroid join + log fallback; `hash(cell_id)` splits; assert cell_id sets equal across LMDBs. Write the missing `recentering_audit.py` (spec §5 gate).
- B5 + B6: full determinism in `breast_pooled_train.py`; fixed class-weight vector across arms; dump per-cell predictions.
- Add the **per-`broken_reason` × per-class drop-rate gate** (B9) — the censoring guardrail.

**Phase 2 — readout rigor (cheap, high-trust):** McNemar + paired-bootstrap CIs + MCC/balanced-acc in the A/B/C readout; ≥3 seeds on the winner; reframe STHELAR as canary.
