...
**Method complementarity.** BANKSY achieves the best T_NK F1 (0.885, +20% vs popV) while popV dominates Blood_vessel (F1=0.891, +43% vs BANKSY). The optimal per-class assignment would exceed any single method. Figure 6 visualises the per-class F1 across a six-method subset for which we have full per-class breakdowns.

![Figure 6 — Cell-type annotation method comparison on STHELAR breast s0 (7-class STHELAR reference). Left: per-class F1 heat map across six methods, sorted by macro F1 ascending. popV (full retrain on STHELAR-derived references) wins overall; Tangram + CellTypist combination is a close second with the highest accuracy. Notable per-class patterns: only popV detects B/Plasma cells (F1=0.51 retrain, 0.21 hub); Specialized class is undetectable by every method (F1=0.00); BANKSY at r=3.0 finds T/NK (F1=0.58) but at r=2.0 misses it (F1=0.02), demonstrating that neighbourhood radius is a sensitive hyperparameter. Right: macro F1 and accuracy bars per method.](figures/fig6_annotation_methods.png)

**Reference-dependence of the BANKSY result.** A separate evaluation on Xenium breast rep1 against Janesick expert GT (3) places BANKSY+scType at F1=0.802 — the best of any single method ([[Annotation Benchmark Results 2026-03|earlier benchmark]]). On the STHELAR 7-class reference shown in Figure 6, BANKSY drops to F1=0.305–0.386. The two evaluations differ in (i) the reference labels (Janesick expert GT vs Tangram-derived STHELAR labels), (ii) the class set (4-broad-class on Xenium vs 7-class on STHELAR including hard categories like B/Plasma, Myeloid, Specialized), and (iii) the platform / gene panel (313-gene Xenium vs ~380-gene STHELAR). The honest reading is that **annotation method ranking is reference-dependent and class-set-dependent**, and a single "best annotator" claim is unsupportable without specifying both reference and class set.
...
The most striking single result is brain: the baseline classifier achieves **0.956 accuracy on brain** when brain is in training, but only **0.200 accuracy when brain is held out**. The endothelial recall on brain drops from approximately 1.0 to 0.168 (precision remains 0.989). This empirically demonstrates that the model learned "looks-like-brain → predict endothelial" as a shortcut rather than learning brain endothelial morphology. The pattern generalises across tissues: in-distribution per-tissue accuracy substantially over-estimates true cell-classification capability when the test tissue is in the training distribution. Figure 7 shows the row-normalised confusion matrix for the in-distribution baseline (the structure that LOTO removes from the model); Figure 8 quantifies *why* per-tissue accuracy diverges from per-tissue macro F1 — Shannon entropy of the per-tissue class distribution.

![Figure 7 — STHELAR multi-tissue baseline confusion matrix (row-normalised recall). Classes sorted by support descending. Strong diagonal on epithelial (0.89), endothelial (0.66) and T cell (0.59); off-diagonal mass concentrates in: macrophage→B cell (0.17, similar small dense nuclei), mast→T cell (0.37), and adipocyte spread across fibroblast (0.39), macrophage (0.28), epithelial (0.17) — the adipocyte class with n=18 test patches is functionally undetected.](figures/fig7_sthelar_confusion.png)

![Figure 8 — Tissue compositional imbalance is what produces the brain paradox. Left: tissue × class composition (test split, true labels) sorted by Shannon entropy ascending. Brain is 93 % endothelial, Shannon H=0.33 bits — almost mono-class, hence accuracy 0.96 with macro F1 only 0.24. Lung is the most diverse (H=2.55) and has the highest macro F1 (0.54). Right: bars showing Shannon entropy, accuracy, and macro F1 per tissue — when entropy is low, accuracy ≫ macro F1 (model exploits the dominant class).](figures/fig8_tissue_composition.png)
...
The eight embedded figures (`docs/figures/fig{1..8}_*.png`, plus `fig9_method_speed_vs_f1.png` referenced in the metrics dossier) are computed directly from `pipeline_output/` JSON via `scripts/manuscript_figures.py` and can be regenerated reproducibly. Captions for these embedded figures are inline at the relevant section. Below are the additional figures planned for the full manuscript figure set.
